/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PromotionNotificationWhereInput } from "./PromotionNotificationWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PromotionNotificationOrderByInput } from "./PromotionNotificationOrderByInput";

@ArgsType()
class PromotionNotificationFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PromotionNotificationWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => PromotionNotificationWhereInput, { nullable: true })
  @Type(() => PromotionNotificationWhereInput)
  where?: PromotionNotificationWhereInput;

  @ApiProperty({
    required: false,
    type: [PromotionNotificationOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [PromotionNotificationOrderByInput], { nullable: true })
  @Type(() => PromotionNotificationOrderByInput)
  orderBy?: Array<PromotionNotificationOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PromotionNotificationFindManyArgs as PromotionNotificationFindManyArgs };
